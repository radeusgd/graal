...
public final class NativeImageHeap implements ImageHeap {
    ...
    HashMap<ObjectInfo, ObjectReachabilityInfo> objectReachabilityInfo = null;
    ...
    public NativeImageHeap(AnalysisUniverse aUniverse, HostedUniverse universe, HostedMetaAccess metaAccess, ImageHeapLayouter heapLayouter) {
        ...
        if (ImageHeapConnectedComponentsFeature.Options.PrintImageHeapConnectedComponents.getValue()) {
            this.objectReachabilityInfo = new HashMap<>();
        }
    ...
    public void addTrailingObjects() {
        ...
        if (usesInternedStrings) {
             ...
             */
            addObject(getMetaAccess().lookupJavaType(String[].class).getHub(), false, InternalReason.InternedStringsTable);
            /*
            ...
            addObject(imageInternedStrings, true, InternalReason.InternedStringsTable);
    ...
    private void addStaticFields() {
        addObject(StaticFieldsSupport.getStaticObjectFields(), false, InternalReason.StaticObjectFields);
        addObject(StaticFieldsSupport.getStaticPrimitiveFields(), false, InternalReason.StaticPrimitiveFields);
    ...
    public ObjectInfo addFillerObject(int size) {
        if (size >= minArraySize) {
            ...
            return addLateToImageHeap(new int[arrayLength], InternalReason.FillerObject);
        } else if (size >= minInstanceSize) {
            return addLateToImageHeap(new FillerObject(), InternalReason.FillerObject);
        } else {
            return null;
        }
    }
    ...
    public final class ObjectInfo implements ImageHeapObject {
         ...
         * in the heap, or an {@link InternalReason}, or a {@link HostedField}.
         */
    ...
    enum ObjectReachabilityGroup {
        ...
        DynamicHubs(1 << 3),
        ImageCodeInfo(1 << 4),
        MethodOrStaticField(1 << 5),
        Other(1 << 6);

        public int flag = 0;

        ObjectReachabilityGroup(int flag) {
            this.flag = flag;
        }

        static int getFlagForObjectInfo(ObjectInfo object, Object firstReason, HashMap<ObjectInfo, ObjectReachabilityInfo> additionalReasonInfoHashMap) {
            int result = 0;
        ...
        static int getByReason(Object reason, HashMap<ObjectInfo, ObjectReachabilityInfo> additionalReasonInfoHashMap) {
